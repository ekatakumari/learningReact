...
const FileList = ({ list,handleAddItem,handleDeleteItem }) => {
            ...
            {list.length > 0 &&
                list.map((data) => (
                    <div key={data.id}>
                       ...
                       <span><img src="https://cdn-icons-png.flaticon.com/512/3161/3161358.png" style={{width:"10px",marginLeft:"5px",cursor:"pointer"}} onClick={()=>handleDeleteItem(data.id)}/></span>
                        {isToggle[data.name] && data.children.length > 0 && <FileList handleAddItem={handleAddItem} list={data.children}  handleDeleteItem={handleDeleteItem}/>}
                    </div>
                ))}
        </div>
    );
